...
# Execute back-and-forth bridging, starting from UNI to OP
print(f"Starting {TIMES} cross-chain transactions between UNI and OP, {AMOUNT_ETH} ETH each time")
for i in range(TIMES):
    print(f"\nCross-chain attempt {i+1}:")
    # UNI -> OP (Now executed first)
    bridge_arb_to_op(AMOUNT_ETH)
    # OP -> UNI (Executed second)
